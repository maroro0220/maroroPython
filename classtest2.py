...
class ViewCalculator:
      def __init__(self):
            self.controlCalcultor=ControlCalculator()
            self.op = { 1:'+', 2:'-', 3:'*', 4:'/' }
      def DisplayCalculator(self):
            ...
            while self.number1 != '0':       
                  ...
                  if self.op_select == 0:
                        break;  
                  self.result = self.controlCalcultor.calculate(self.number1, self.number2, self.op_select )
                  print( '\n{0:^6} {2:^3} {1:^6} = {3:<.2f}'.format( self.number1, self.number2, self.op[ self.op_select ], self.result ) )
                  print( '\n' )
                  self.number1 = input( 'Input number1 (0:end): ' )     
            return
